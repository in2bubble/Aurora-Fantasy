...
/* Aurora Fantasy - gbuffers_weather.vsh
   ...
   Clean-room vertex displacement native to Aurora Fantasy.
...
void main() {
    ...
    // Aurora Fantasy wind-shear displacement
    vec3 viewWind = normalize(mat3(gl_ModelViewMatrix) * normalize(vec3(0.82, 0.0, 0.57)));
    float gust = 0.86 + 0.14 * sin(persistentTimeSeconds * 0.72 + gl_Vertex.x * 0.041 + gl_Vertex.z * 0.033);
