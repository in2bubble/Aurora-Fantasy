/* Aurora Fantasy - volumetric_clouds_end.glsl
...
vec3 get_end_cloud(vec3 view_vector, vec3 block_color, float bright, float dither, vec3 base_pos, int samples) {
    // Reduced brightness effect to prevent white glare
    block_color.rgb *= clamp(bright * 0.15 + ((dither - .5) * .05), 0.0, 1.0) * .2 + 1.0;
    ...
    if (view_vector.y > 0.0) {  // Vista sobre el horizonte
        ...
        // Removed white mixing to prevent glare - keep natural End colors
    }
