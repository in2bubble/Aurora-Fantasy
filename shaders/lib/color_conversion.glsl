...
vec3 rgb_to_xyz_acc(vec3 rgb) {
    vec3 linear_rgb;
    vec3 mask = vec3(greaterThan(rgb, vec3(0.04045)));
    linear_rgb = mix(rgb / 12.92, pow((rgb + 0.055) / 1.055, vec3(2.4)), mask);
    ...
    return srgb_to_xyz_matrix * linear_rgb;
}
...
vec3 xyz_to_rgb_acc(vec3 xyz) {
    ...
    vec3 rgb_linear = xyz_to_srgb_matrix * xyz;
    
    vec3 corrected_rgb;

    corrected_rgb.r = (rgb_linear.r <= 0.0031308) ? 12.92 * rgb_linear.r : 1.055 * pow(rgb_linear.r, 1.0 / 2.4) - 0.055;
    corrected_rgb.g = (rgb_linear.g <= 0.0031308) ? 12.92 * rgb_linear.g : 1.055 * pow(rgb_linear.g, 1.0 / 2.4) - 0.055;
    corrected_rgb.b = (rgb_linear.b <= 0.0031308) ? 12.92 * rgb_linear.b : 1.055 * pow(rgb_linear.b, 1.0 / 2.4) - 0.055;
    
    return clamp(corrected_rgb, 0.0, 1.0);
}
